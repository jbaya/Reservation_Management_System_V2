...
import {
  getCategories, saveCategory, getBookings, updateCategory, deleteCategory,
  getRooms, getAllRoomNumbers, saveRoom, deleteRoom, updateRoom,
  updateRoomCategory, getAgents, saveAgent, deleteAgent,
  getThirdParties, saveThirdParty, deleteThirdParty,
  getSeasons, saveSeason, updateSeason, deleteSeason,
  getRates, getFloors, saveFloor, deleteFloor,
  updateBooking, saveBooking, getSpecialDates
} from './api.js';
...
function App() {
  ...
  // Re-sort rooms when count changes
  useEffect(() => {
    setRooms(prev => {
  ...
  useEffect(() => {
    ...
    getRooms().then(data => {
      if (data?.length > 0) setRooms(sortRoomList(data));
    }).catch(() => {});

    getAgents().then(data => setTravelAgents(data || [])).catch(() => {});
    getThirdParties().then(data => setThirdParties(data || [])).catch(() => {});
    getSeasons().then(data => setSeasons(data || [])).catch(() => {});
    getRates().then(data => setTravelAgentRates(data || [])).catch(() => {});
    getBookings().then(data => setBookings(data || [])).catch(() => {});
    ...
    getSpecialDates().then(data => setSpecialDates(data || [])).catch(() => {});
  }, []);
  ...
  const handleSaveBooking = useCallback(async (data) => {
    ...
    try {
      ...
      if (isEdit) {
        await updateBooking(data.id, { ...data, timestamp: now });
      } else {
        await saveBooking({ ...data, id: data.id || `b${Date.now()}`, timestamp: now });
      }
  ...
  const handleAddCategory = useCallback(async (name, roomCount, fromRoom = null, toRoom = null, floor = '1') => {
    ...
    if (roomCount && !isNaN(roomCount) && roomCount > 0) {
      ...
      if (fromRoom !== null && toRoom !== null) {
        ...
        const parsedNumbers = allRoomNumbers.map(n => parseInt(n, 10)).filter(n => !isNaN(n));
    ...
    try {
      ...
      for (const room of newRooms) await saveRoom(room);

      setCategoryColors(prev => ({ ...prev, [name]: { ...color, floor } }));
  ...
  const handleEditCategory = useCallback(async (oldName, newName, newColor, newRoomCount, newFloor) => {
    ...
    if (isNaN(targetCount) || targetCount < 0) { alert('Invalid room count'); return; }

    try {
      const allCats = await getCategories();
      const catRow  = allCats.find(r => r.category === oldName);
      if (catRow) await updateCategory(catRow.id, newName, targetCount, newColor, newFloor);

      if (oldName !== newName || newFloor) await updateRoomCategory(oldName, newName, newFloor);

      const currentRooms = rooms.filter(r => r.category === oldName || r.category === newName);
      const currentCount = currentRooms.length;

      if (targetCount > currentCount) {
        const catRoomNumbers = currentRooms.map(r => parseInt(r.name, 10)).filter(n => !isNaN(n));
        ...
        for (let i = 0; i < targetCount - currentCount; i++) {
          ...
          await saveRoom({ name: String(nextNum), category: newName, floor: newFloor || '1' });
      ...
      if (targetCount < currentCount) {
        ...
        if (hasActive) { alert('Cannot reduce — some rooms have active bookings!'); return; }
        for (const rn of roomsToRemove) await deleteRoom(rn);
      }
      ...
      setCategoryColors(prev => {
        const updated = { ...prev };
        updated[newName] = { bg: `rgba(${r}, ${g}, ${b}, 0.18)`, border: newColor, num_rooms: targetCount, floor: newFloor };
  ...
  const handleDeleteCategory = useCallback(async (name) => {
    try {
      ...
      if (hasActiveBookings) { alert('Cannot delete category with active bookings'); return; }

      const allCats = await getCategories();
      const catRow  = allCats.find(c => c.category === name);
      if (!catRow) { alert('Category not found'); return; }

      const categoryRooms = rooms.filter(r => r.category === name);
      for (const room of categoryRooms) await deleteRoom(room.name);
  ...
  const handleAddRoom = useCallback(async (room) => {
    try {
      await saveRoom({ name: room.name, category: room.category, floor: room.floor || '1', capacity: room.capacity || 2 });
      const freshRooms = await getRooms();
      setRooms(sortRoomList(freshRooms));
    } catch (err) {
  ...
  const handleDeleteRoom = useCallback(async (name) => {
    if (bookings.some(b => b.roomName === name && !['cancelled', 'no-show'].includes(b.status))) {
      alert('Cannot delete room with active bookings'); return;
    }
  ...
  const handleUpdateRoom = useCallback(async (oldRoomNo, updatedRoom) => {
    try {
      await updateRoom(oldRoomNo, { roomNo: updatedRoom.roomNo, category: updatedRoom.category, floor: updatedRoom.floor, capacity: updatedRoom.capacity || 2 });
      const freshRooms = await getRooms();
      setRooms(sortRoomList(freshRooms));
    } catch (err) {
  ...
  const sidebarMenus = [
    {
      key: 'room', label: 'Room Management', icon: <BedDouble size={16} />,
      children: [
    ...
    {
      key: 'user-management', label: 'User Management', icon: <UserCog size={16} />,
      children: [
        { key: 'users', label: 'Users', icon: <Users size={14} /> },
      ],
    },
    {
      key: 'reservation', label: 'Reservation', icon: <CalendarDays size={16} />,
      children: [
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#f0f2f5', overflow: 'auto', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
        ...
        <div style={{ width: sidebarOpen ? 240 : 0, minWidth: sidebarOpen ? 240 : 0, background: '#1e2a3a', transition: 'width 0.25s ease, min-width 0.25s ease', overflow: 'hidden', flexShrink: 0, display: 'flex', flexDirection: 'column', zIndex: 100, boxShadow: sidebarOpen ? '4px 0 16px rgba(0,0,0,0.2)' : 'none' }}>
          <div style={{ width: 240, flex: 1, overflowY: 'auto' }}>
            ...
            <div
              ...
              style={{ padding: '10px 18px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10, color: activePage === null ? '#64b5f6' : 'rgba(255,255,255,0.7)', background: activePage === null ? 'rgba(100,181,246,0.1)' : 'transparent', borderLeft: activePage === null ? '3px solid #64b5f6' : '3px solid transparent', fontSize: '0.85rem', fontWeight: activePage === null ? 700 : 500 }}
            >
            ...
            {sidebarMenus.map(menu => (
              <div key={menu.key}>
                <div
                  ...
                  style={{ padding: '10px 18px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'space-between', color: 'rgba(255,255,255,0.85)', background: expandedMenu === menu.key ? 'rgba(255,255,255,0.05)' : 'transparent', fontSize: '0.85rem', fontWeight: 700, borderTop: '1px solid rgba(255,255,255,0.06)' }}
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>{menu.icon}<span>{menu.label}</span></div>
                  ...
                  <div style={{ background: 'rgba(0,0,0,0.15)' }}>
                    {menu.children.map(child => (
                      <div
                        key={child.key}
                        onClick={() => {
                          if (child.key === 'new-reservation' || child.key === 'multi-room-reservation') setEditingBooking(null);
                          setActivePage(child.key);
                          setSidebarOpen(true);
                        }}
                        style={{ padding: '8px 18px 8px 36px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 8, color: activePage === child.key ? '#64b5f6' : 'rgba(255,255,255,0.6)', background: activePage === child.key ? 'rgba(100,181,246,0.12)' : 'transparent', borderLeft: activePage === child.key ? '3px solid #64b5f6' : '3px solid transparent', fontSize: '0.8rem', fontWeight: activePage === child.key ? 700 : 400 }}
                      >
                        <span style={{ color: activePage === child.key ? '#64b5f6' : 'rgba(255,255,255,0.5)', flexShrink: 0, display: 'flex' }}>{child.icon}</span>
                        {child.label}
                      </div>
                    ))}
                  </div>
            ...
            <div style={{ marginTop: 'auto', borderTop: '1px solid rgba(255,255,255,0.08)', padding: 16 }}>
              <button
                onClick={() => { localStorage.removeItem('rms_loggedIn'); setLoggedUser(null); }}
          ...
          {activePage === null ? (
            <>
              ...
              <div style={{ background: '#fff', borderBottom: '1px solid #e8eaed', padding: '6px 18px', display: 'flex', gap: 16, alignItems: 'center', flexShrink: 0, flexWrap: 'wrap' }}>
                ...
                <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid #ddd', fontSize: '0.7rem', fontWeight: 600, cursor: 'pointer' }}>
                  {['all','confirmed','tentative','checked-in','checked-out','cancelled','blocked'].map(s => (
                    <option key={s} value={s}>{s}</option>
                  ))}
                </select>

                <select value={guestTagFilter} onChange={e => setGuestTagFilter(e.target.value)} style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid #ddd', fontSize: '0.7rem', fontWeight: 600, cursor: 'pointer' }}>
              ...
              <main style={{ flex: 1, padding: '8px 14px', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                {filteredBookings.length === 0 && debouncedSearch ? (
                  ...
                  <CalendarView
                    ...
                    categoryColors={categoryColors}
                    currentUser={loggedUser?.name || 'Staff'}
            ...
            <div style={{ flex: 1, overflowY: 'auto', background: '#f8f9fa' }}>
              ...
              {activePage === 'room-category' && (
                <RoomCategoryPage categoryColors={categoryColors} rooms={rooms} floors={floors} onAddCategory={handleAddCategory} onDeleteCategory={handleDeleteCategory} onEditCategory={handleEditCategory} />
              )}
              {activePage === 'room-no' && (
                <RoomNoPage rooms={rooms} floors={floors} categoryColors={categoryColors} onAddRoom={handleAddRoom} onDeleteRoom={handleDeleteRoom} onUpdateRoom={handleUpdateRoom} />
              )}
              ...
              {activePage === 'view-reservation' && (
                <ViewReservationPage bookings={bookings} rooms={rooms} categoryColors={categoryColors} currentUser={loggedUser} travelAgents={travelAgents} seasons={seasons} travelAgentRates={travelAgentRates} onUpdateBooking={handleUpdateBooking} />
              )}
              ...
              {activePage === 'travel-agent'     && <TravelAgentPage agents={travelAgents} thirdParties={thirdParties} onAgentsChange={setTravelAgents} onThirdPartyChange={setThirdParties} />}
              {activePage === 'season-config'    && <SeasonConfigPage seasons={seasons} onSeasonsChange={setSeasons} />}
              {activePage === 'travel-agent-rate' && <TravelAgentRateConfig agents={travelAgents} seasons={seasons} categoryColors={categoryColors} travelAgentRates={travelAgentRates} onRatesChange={setTravelAgentRates} />}
              {activePage === 'room-floor'       && <FloorPage floors={floors} rooms={rooms} floorFilter={floorFilter} setFloorFilter={setFloorFilter} setFloors={setFloors} categoryColors={categoryColors} />}
              {activePage === 'special-dates'    && <SpecialDatesPage specialDates={specialDates} onSpecialDatesChange={setSpecialDates} />}
      ...
      <DncManager open={dncOverrideOpen} booking={dncBooking} targetRoom={dncTargetRoom} onApprove={handleDncApprove} onCancel={handleDncCancel} />
    </div>
  );
}
