...
const get  = (url) => fetch(url, { headers: authHeaders() }).then(toJson);
const del  = (url) => fetch(url, { method: 'DELETE', headers: authHeaders() }).then(toJson);
const post = (url, body) => fetch(url, { method: 'POST',  headers: authHeaders(), body: JSON.stringify(body) }).then(toJson);
const put  = (url, body) => fetch(url, { method: 'PUT',   headers: authHeaders(), body: JSON.stringify(body) }).then(toJson);
