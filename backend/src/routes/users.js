  ...
  try {
    ...
    const { rows } = await db.query(
      `INSERT INTO users 
       ...
       RETURNING *`,
      [full_name, gender, mobile, email, designation, username, password, user_type, status || 'active']
    );
...
router.put('/:id', async (req, res, next) => {
  try {
    ...
    const { rows } = await db.query(
      ...
      [full_name, gender, mobile, email, designation, username, password, user_type, status, req.params.id]
    );
